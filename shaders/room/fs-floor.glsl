    ...
    void main() {
        ...
        float distance = min( 1. , 20. / length( vPosition.xy ) );



        gl_FragColor = vec4( vec3( 3.0 , .8 , 0.0 ) * base.rgb * distance * abs(vDisplacement),  1. );
        //gl_FragColor = vec4( vDisplacement * base.rgb * 20. / length(abs(dFromCenter)),  .1 );
